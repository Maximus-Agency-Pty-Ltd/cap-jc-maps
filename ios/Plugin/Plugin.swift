...
//                } else {
                    if(coordinates.count > 1) {
                        ...
                        CATransaction.setCompletionBlock({
                          call.success([ "cameraSet": true ])
                        })
                        self.mapViewController.GMapView.animate(with: GMSCameraUpdate.fit(gmsBounds, withPadding: 50.0))
                        CATransaction.commit()
                    } else {
                        self.mapViewController.GMapView.isMyLocationEnabled = true;
                        self.mapViewController.GMapView.settings.compassButton = true;
                        self.mapViewController.GMapView.setMinZoom(10, maxZoom: zoom);
                        self.mapViewController.GMapView.animate(to: camera);
                        call.success([ "cameraSet": true ])
                    }
//                }
    ...
    @objc func addPolyline(_ call: CAPPluginCall) {
        ...
        DispatchQueue.main.async {
          ...
          //    }
            ...
            let animatePolyline = AnimatePolyline(mapView: self.mapViewController.GMapView)
            animatePolyline.strokeColor = .blue
            animatePolyline.route = path
            animatePolyline.startAnimation()

//            var route = getRoute(path)
//            drawRouteOnMap(route)
//            animatePolylineWithGradient()
             // let polyline = GMSPolyline(path: path)
             // polyline.map = self.mapViewController.GMapView
